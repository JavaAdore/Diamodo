...
public class DiamodoManagementImpl implements DiamodoManagement {
	...
	@Override
	public List<FriendSearchResult> findFriends(String searcherId, FriendsSearchCriteriaDTO friendsSearchCriteriaDTO)
	...
	@Override
	public Friendship sendFriendRequest(String clientId, String friendId) throws DiamodoCheckedException {
		Friendship friendShip = friendshipService.sendFriendRequest(clientId, friendId);
		logger.info("friendship sent successfully sender is :" + clientId + " reciever is : " + friendId);
		pushFriendshipRecievedNotification(friendId, clientId);
		return friendShip;
	}

	@Override
	public String acceptFriendShip(String clientId, String friendshipId) throws DiamodoCheckedException {

		return friendshipService.acceptFriendShip(clientId, friendshipId);
	}

	@Override
	public String rejectFriendship(String clientId, String friendshipId) throws DiamodoCheckedException {
	...
	@Override
	public ClientInfo updateProfile(String clientId, UpdateProfileDTO updateProfile) throws DiamodoCheckedException {
		return diamodoClientService.updateProfilePicture(clientId, updateProfile);
	}

	@Override
	public void validateConversationMemebers(String clientId, String conversationId) throws DiamodoCheckedException {
	...
	public void pushFriendshipRecievedNotification(String recieverId, String senderId) {
		...
		if (Utils.isNotEmpty(senderDTO)) {
			...
			UserWithDeviceInfo userWithDeviceInfo = diamodoClientService.getClientWithDeviceInfo(recieverId);
			logger.info("getting reciver with device " + userWithDeviceInfo);

			if (Utils.isNotEmpty(userWithDeviceInfo) && userHasValidDevice(userWithDeviceInfo.getClientDevice())) {

				logger.info("reciever passed all validations and start pushing him / her friendship request");
	...
	private boolean userHasValidDevice(ClientDevice clientDevice) {
		...
		boolean result = clientDevice != null && clientDevice.isAcceptedDevice()
	...
	public CreateConversationResponse createNewConversation(String clientId,
			final CreateConversationRequest createConversationRequest) throws DiamodoCheckedException {
		CreateConversationResponse createConversationResponse = chatMessageService.createNewConversation(clientId,
		...
		taskExecutor.execute(new Runnable() {
			...
			@Override
			public void run() {
				sendPushNotificationForConversationAdding(clientId,createConversationResponse.getId(), createConversationRequest.getMemberIds());
			}
	...
	@Override
	public void assignNewAdmin(String clientId, AssignNewAdminsDTO assignNewAdminsDTO) throws DiamodoCheckedException {
	...
	@Override
	public void leaveConversation(String clientId, String conversationId) throws DiamodoCheckedException {
	...
	public void inviteMemberToConversation(String clientId,
		...
		taskExecutor.execute(new Runnable() {
			@Override
			public void run() {
				sendPushNotificationForConversationAdding(clientId,
	...
	protected void sendPushNotificationForConversationAdding(String clientId, String conversationId,
			Set<String> memberIds) {
		ClientInfo clientInfo = diamodoClientService.getClientById(clientId);
		String conversationName = chatMessageService.getConversationName(conversationId);
		sendPushNotificationForConversationAdding(memberIds, clientInfo, conversationId, conversationName);

	}

	private void sendPushNotificationForConversationAdding(Set<String> memberIds, ClientInfo clientInfo,
			String conversationId, String conversationName) {

		memberIds.parallelStream().forEach(member -> {
			if (!member.equalsIgnoreCase(clientInfo.getId())) {
				UserWithDeviceInfo userWithDeviceInfo = diamodoClientService.getClientWithDeviceInfo(member);
				logger.info("getting reciver with device " + userWithDeviceInfo);

				if (Utils.isNotEmpty(userWithDeviceInfo) && userHasValidDevice(userWithDeviceInfo.getClientDevice())) {
					logger.info(
							"reciever passed all validations and start pushing him / her add to conversation push notification");
					pushNotificationManager.pushNotificationForConversationAdding(clientInfo, userWithDeviceInfo,
	...
	public void sendPushNotificationForChatMessage(LightConversation conversation,
		...
		if (Utils.isNotEmpty(senderDTO)) {
			...
			if (Utils.isNotEmpty(userWithDeviceInfo) && userHasValidDevice(userWithDeviceInfo.getClientDevice())) {
				...
				logger.info("reciever passed all validations and start pushing him / her friendship request");
				pushNotificationManager.pushOfflineMessageNotification(userWithDeviceInfo, senderDTO, conversation,
  ...
  @Override
  public String deleteFriendship(String clientId, String friendshipId) throws DiamodoCheckedException {
    
   String otherPartId =  friendshipService.deleteFriendship(clientId, friendshipId);
   return otherPartId;
